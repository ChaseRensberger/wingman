// Package openaicompat implements wingmodels.Model for any provider that
...
//
// Stream mapping (Chat Completions SSE -> wingmodels.StreamPart):
//
...
func (c *Client) Info() wingmodels.ModelInfo {
	if info, ok := catalog.Get(c.catalogProvider, c.model); ok {
		info.API = wingmodels.APIOpenAICompletions
	...
	return wingmodels.ModelInfo{
		...
		ID:       c.model,
		API:      wingmodels.APIOpenAICompletions,
		BaseURL:  c.baseURL,
	}
}
...
func (c *Client) buildRequest(req wingmodels.Request) chatRequest {
...
// toWireMessages converts wingmodels messages to OpenAI Chat Completions format.
//
// Role mapping:
//   - user      -> {role:"user", content:[{type:"text",text},...]}
//   - assistant -> {role:"assistant", content:"...", tool_calls:[...]}
//   - tool      -> {role:"tool", tool_call_id, content} (one per ToolResultPart)
func (c *Client) toWireMessages(req wingmodels.Request) []chatMessage {
	var out []chatMessage
...
func toolResultText(out []wingmodels.Part) string {
...
type inFlightToolCall struct {
	...
	streamID string // wingmodels stream-part id
}

func newStreamParser(out *wingmodels.EventStream[wingmodels.StreamPart, *wingmodels.Message], origin *wingmodels.MessageOrigin, providerID string) *streamParser {
	return &streamParser{
		out:        out,
		origin:     origin,
		providerID: providerID,
...
func (p *streamParser) handleChunk(data string) bool {
	...
	if !p.startEmitted && chunk.ID != "" {
		p.responseMeta = wingmodels.ResponseMetadata{ID: chunk.ID}
	...
	if delta.ReasoningContent != nil && *delta.ReasoningContent != "" {
		if !p.reasoningStarted {
			...
			p.out.Push(wingmodels.ReasoningStartPart{ID: p.reasoningID})
			p.reasoningStarted = true
		}
		p.reasoningBuf.WriteString(*delta.ReasoningContent)
		p.out.Push(wingmodels.ReasoningDeltaPart{ID: p.reasoningID, Delta: *delta.ReasoningContent})
	}
	...
	if delta.Content != nil && *delta.Content != "" {
		if !p.textStarted {
			...
			if p.reasoningStarted {
				...
				p.out.Push(wingmodels.ReasoningEndPart{ID: p.reasoningID})
		...
		p.out.Push(wingmodels.TextDeltaPart{ID: p.textID, Delta: *delta.Content})
	}
	...
	for _, tc := range delta.ToolCalls {
		if _, exists := p.toolCalls[tc.Index]; !exists {
			...
			p.out.Push(wingmodels.ToolInputStartPart{ID: tc.ID, ToolName: tc.Function.Name})
		}
		...
		if tc.Function.Arguments != "" {
			...
			p.out.Push(wingmodels.ToolInputDeltaPart{ID: call.id, Delta: tc.Function.Arguments})
		}
	}
...
func (p *streamParser) flush() {
	...
	for _, i := range keys {
		...
		p.out.Push(wingmodels.ToolInputEndPart{ID: call.id})
		p.out.Push(wingmodels.ToolCallPart_{ID: call.id, ToolName: call.name, Input: input})
	}
}
...
func mapFinishReason(r string) wingmodels.FinishReason {
	switch r {
	case "stop":
...
func runStream(ctx context.Context, resp *http.Response, out *wingmodels.EventStream[wingmodels.StreamPart, *wingmodels.Message], origin *wingmodels.MessageOrigin, providerID string) {
