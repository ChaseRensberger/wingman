...
test("uses the foreground daemon default while loading service credentials", () => {
  const dir = stateDir();
	...
	expect(readDaemonProxy(dir, config)).toEqual({ target: "http://127.0.0.1:2323", username: "chase", password: "foreground-password" });
});
...
test("uses a managed daemon registration when available", () => {
  ...
  writeFileSync(join(dir, "registration.json"), JSON.stringify({ url: "http://127.0.0.1:4545" }));
	...
	expect(readDaemonProxy(dir, config)).toEqual({ target: "http://127.0.0.1:4545", username: "wingman", password: "managed-password" });
});
