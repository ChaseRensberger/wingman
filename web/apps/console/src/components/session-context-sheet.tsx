...
function RequestManifest({ trace }: { trace?: CallTrace }) {
  ...
  return (
    <Collapsible className="mt-3 rounded-md border bg-muted/20">
      <CollapsibleTrigger className="px-2.5 py-2 text-xs font-medium hover:no-underline">
        Request manifest
      </CollapsibleTrigger>
      <CollapsibleContent className="border-t px-2.5 py-2 text-xs text-muted-foreground">
        <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1.5">
          <span>Capabilities</span>
          <span>{trace.capabilities.thinking ? "Reasoning enabled" : "Default"}</span>
          <span>Current date</span>
          <span>{trace.runtime.current_date ? "Injected" : "Not injected"}</span>
          <span>Reasoning summary</span>
          <span>{trace.lowered?.reasoning_summary_auto ? "Auto" : "Not requested"}</span>
          <span>System prompt</span>
          <span className="font-mono">
            {trace.system.sha256.slice(0, 12)} · {trace.system.bytes} bytes
          </span>
          <span>Messages</span>
          <span>
            {trace.messages.count} ·{" "}
            {Object.entries(trace.messages.part_kinds)
              .map(([kind, count]) => `${count} ${kind}`)
              .join(", ") || "no parts"}
          </span>
          <span>Tools</span>
          <span>
            {trace.tools?.length ? trace.tools.map((tool) => tool.name).join(", ") : "None"}
          </span>
        </div>
      </CollapsibleContent>
    </Collapsible>
...
export function SessionContextSheet({ session, calls }: { session: Session; calls: ModelCall[] }) {
  ...
  const hasUsage = calls.some(
    (call) => call.total_tokens > 0 || call.input_tokens > 0 || call.output_tokens > 0,
  );
      ...
      <Tooltip>
        <TooltipTrigger
          render={
            <Button
              ...
              variant="ghost"
              size="icon-sm"
              aria-label="View session context and usage"
        ...
        <TooltipContent className="w-44 bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10">
          <div className="grid grid-cols-[1fr_auto] gap-x-4 gap-y-1.5">
            ...
            <span>{hasCost ? formatCost(totalCost) : "Not reported"}</span>
            <span className="text-muted-foreground">Usage</span>
            <span>{hasUsage ? `${Math.round(contextPercent)}%` : "Not reported"}</span>
            <span className="text-muted-foreground">Tokens</span>
            <span>{hasUsage ? formatTokenCount(contextTokens) : "Not reported"}</span>
          </div>
        </TooltipContent>
      </Tooltip>
      <SheetContent className="w-full gap-4 p-0 sm:max-w-xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-6">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Stat
              label="Context usage"
              value={hasUsage ? `${Math.round(contextPercent)}%` : "Not reported"}
            />
            ...
            <Stat
              label="Input tokens"
              value={hasUsage ? formatTokenCount(totalInput) : "Not reported"}
            />
            <Stat
              label="Output tokens"
              value={hasUsage ? formatTokenCount(totalOutput) : "Not reported"}
            />
            <Stat
              label="Reasoning tokens"
              value={hasUsage ? formatTokenCount(totalReasoning) : "Not reported"}
            />
            <Stat
              label="Cached tokens"
              value={hasUsage ? formatTokenCount(totalCached) : "Not reported"}
            />
          </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="truncate text-sm font-medium">
                          {call.model_ref || call.model_id || "Unknown model"}
                        </div>
                        <div className="mt-0.5 truncate text-xs text-muted-foreground">
                          {call.run_id ? `${call.run_id} · ` : ""}Step {call.step} · attempt{" "}
                          {call.attempt} · {call.status}
                          {call.finish_reason ? ` · ${call.finish_reason}` : ""}
                        </div>
                      </div>
                      <span className="shrink-0 text-xs text-muted-foreground">
                        {call.cost === undefined ? "Not reported" : formatCost(call.cost)}
                      </span>
                    </div>
                    {call.total_tokens > 0 || call.input_tokens > 0 || call.output_tokens > 0 ? (
                      <div className="mt-2 grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                        <span>In {formatTokenCount(call.input_tokens)}</span>
                        <span>Out {formatTokenCount(call.output_tokens)}</span>
                        <span>{Math.round(call.context_percent ?? 0)}% context</span>
                      </div>
                    ) : (
