...
import type { Message, ModelCall, PermissionRequest, ToolActivity, ToolCallPart, ToolResultPart } from "@/lib/types";
...
function PermissionRequestCard({ request, replying, onReply }: { request: PermissionRequest; replying: boolean; onReply: Props["onReplyPermission"] }) {
	const headingID = `permission-request-${request.id}`;
	return <section aria-labelledby={headingID} className="mx-4 my-5 rounded-lg border border-border bg-muted/30 p-4 text-sm sm:mx-6"><div className="font-medium" id={headingID}>Permission requested</div><div className="mt-2"><span className="text-muted-foreground">Action: </span><span className="font-medium">{request.action}</span></div><ul className="mt-2 space-y-1" aria-label="Requested resources">{request.resources.map((resource, index) => <li key={`${resource}:${index}`} className="break-all text-muted-foreground"><code className="rounded bg-muted px-1 py-0.5 font-mono text-xs text-foreground">{resource}</code></li>)}</ul><p className="mt-3 text-xs text-muted-foreground">Always allow is remembered for this session.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row sm:justify-end"><Button size="sm" type="button" disabled={replying} onClick={() => onReply(request.id, "once")}>Allow once</Button><Button size="sm" variant="outline" type="button" disabled={replying} onClick={() => onReply(request.id, "always")}>Always allow</Button><Button size="sm" variant="destructive" type="button" disabled={replying} onClick={() => onReply(request.id, "reject")}>Reject</Button></div></section>;
}
...
export function SessionTranscript({
	...
	const showThinking = shouldShowThinking(isStreaming, Boolean(streamingText || streamingReasoning || hasActiveTool || permissionRequests.length));
	return (
		<div className="relative min-h-0 flex-1" onPointerEnter={() => scroll.setIsHovered(true)} onPointerLeave={() => scroll.setIsHovered(false)}>
			<div ref={scroll.scrollRef} onScroll={scroll.handleScroll} onKeyDown={scroll.handleKeyDown} tabIndex={0} role="region" aria-label="Session transcript" data-scrollable className="h-full overflow-y-auto outline-none focus-visible:ring-3 focus-visible:ring-ring/50 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
				<div className="mx-auto flex min-h-full w-full max-w-4xl flex-col px-3 pt-5 pb-0 sm:px-4 sm:pt-6">
					{jsonMode ? (
						<div className="px-4 pb-6 sm:px-6"><RawMessages messages={rawMessages} />{permissionRequests.map((request) => <PermissionRequestCard key={request.id} request={request} replying={permissionRepliesInFlight.has(request.id)} onReply={onReplyPermission} />)}</div>
						...
						<div>
							...
							{permissionRequests.map((request) => <PermissionRequestCard key={request.id} request={request} replying={permissionRepliesInFlight.has(request.id)} onReply={onReplyPermission} />)}
