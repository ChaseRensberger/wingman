...
function normalizeTool({ call, result, activity }: Props): ToolView {
  const status =
    activity?.status ?? (result ? (result.is_error ? "error" : "completed") : "pending");
  ...
  return {
    ...
    structured:
      result?.structured ??
      result?.metadata?.structured_content ??
      activity?.metadata?.structured_content,
...
export function ToolActivityItem({ compact = false, ...props }: Props & { compact?: boolean }) {
  ...
  if (["apply_patch", "edit", "write"].includes(view.call.name))
    return <FileMutationTool view={view} />;
  if (["read", "grep", "glob", "webfetch", "websearch"].includes(view.call.name))
    return <TimelineTool view={view} compact={compact} />;
...
function StatusIcon({ status }: { status: ToolView["status"] }) {
  if (status === "pending" || status === "running")
    return <CircleNotchIcon className="size-3.5 animate-spin" />;
...
function sourceLabel(view: ToolView) {
  ...
  const remote =
    typeof view.metadata.remote_name === "string"
      ? humanizeToolName(view.metadata.remote_name)
      : "";
    ...
    >
      ...
      {changes && (
        <span className="ml-auto flex shrink-0 gap-2 font-mono text-xs">
          <span className="text-[var(--console-diff-add-foreground)]">+{changes.additions}</span>
          <span className="text-[var(--console-diff-delete-foreground)]">-{changes.deletions}</span>
        </span>
      )}
      {duration && (
        <span className={cn("shrink-0 text-[11px] tabular-nums", !changes && "ml-auto")}>
          {duration}
        </span>
      )}
    </div>
...
function BashTool({ view }: { view: ToolView }) {
  ...
  return (
    <div
      className={cn(
        "my-2 overflow-hidden rounded-lg border bg-[var(--console-command-background)] text-[var(--console-command-foreground)]",
        view.status === "error"
          ? "border-destructive/50"
          : "border-[var(--console-command-border)]",
      )}
    >
      ...
      {workDir && (
        <div className="border-b border-[var(--console-command-border)] px-3 py-1 font-mono text-[10px] text-[var(--console-command-muted)]">
          {workDir}
        </div>
      )}
      {(output || view.error) && (
        <pre
          data-scrollable
          tabIndex={0}
          className="max-h-96 overflow-auto whitespace-pre-wrap px-3 py-2 font-mono text-xs leading-5"
        >
          <code>
            {visibleOutput}
            {view.error && view.error !== output ? `${output ? "\n" : ""}${view.error}` : ""}
          </code>
        </pre>
      )}
      ...
      {collapsed.overflow && (
        <button
          ...
          className="w-full border-t border-[var(--console-command-border)] px-3 py-1.5 text-left text-[11px] text-[var(--console-command-muted)] hover:text-[var(--console-command-foreground)]"
        >
  ...
  return (
    <div
      className={cn(
        ...
        view.status === "error" ? "border-destructive" : "border-border",
      )}
    >
      <div className="flex min-h-7 items-center">
        <ToolHeader
          view={view}
          title={toolSummary(view.call)}
          changes={files.length > 0 ? changes : undefined}
        />
      </div>
...
function TimelineTool({ view, compact }: { view: ToolView; compact: boolean }) {
  const baseTitle =
    view.call.name === "websearch" && typeof view.metadata.provider === "string"
  ...
  const count =
    view.call.name === "grep"
      ? view.metadata.matches
      : view.call.name === "glob"
        ? view.metadata.count
        : view.call.name === "websearch"
          ? view.metadata.numResults
          : undefined;
  const countLabel =
    typeof count === "number" ? `${count} ${count === 1 ? "result" : "results"}` : "";
  const title = countLabel ? `${baseTitle} (${countLabel})` : baseTitle;
  return (
    <div className={compact ? "my-0" : "my-1.5"}>
      <div className="flex min-h-7 items-center">
...
function CompactTool({ view }: { view: ToolView }) {
  ...
  const hasDetails = Boolean(
    output || view.error || structured || Object.keys(view.call.input).length > 0,
  );
  ...
  const countLabel =
    typeof count === "number"
      ? `${count} ${view.call.name === "websearch" ? "results" : count === 1 ? "match" : "matches"}`
      : "";
  const title = countLabel ? `${baseTitle} (${countLabel})` : baseTitle;
  if (!hasDetails) {
    return (
      <div className="my-1.5 flex min-h-7 items-center">
        <ToolHeader view={view} title={title} />
      </div>
    );
  }
  return (
    <Collapsible defaultOpen={defaultOpen} className="my-1.5">
      <CollapsibleTrigger
        disabled={!hasDetails}
        className="min-h-7 py-0.5 hover:no-underline disabled:opacity-100"
      >
        <ToolHeader view={view} title={title} />
      </CollapsibleTrigger>
      {preview.output && (
        <pre className="ml-8 max-h-20 overflow-hidden whitespace-pre-wrap font-mono text-xs leading-5 text-muted-foreground">
          {preview.output}
        </pre>
      )}
      {view.error && <ErrorText error={view.error} />}
      {hasDetails && (
        <CollapsibleContent className="pb-0">
          <div className="ml-8 mt-1 grid gap-2 border-l pl-3">
            {read?.path && (
              <div className="break-all font-mono text-xs text-muted-foreground">{read.path}</div>
            )}
            {output && <DetailBlock label="Output">{output}</DetailBlock>}
            <ToolOutputParts parts={view.outputParts} structured={structured} />
            {Object.keys(view.call.input).length > 0 && (
              <DetailBlock label="Input">{JSON.stringify(view.call.input, null, 2)}</DetailBlock>
            )}
            {sourceLabel(view) && (
              <div className="text-[11px] text-muted-foreground">{sourceLabel(view)}</div>
            )}
          </div>
        </CollapsibleContent>
      )}
...
function OutputPreview({ view }: { view: ToolView }) {
  ...
  if (!text) return null;
  return (
    <pre className="mt-1 max-h-24 overflow-auto whitespace-pre-wrap font-mono text-xs leading-5 text-muted-foreground">
      {collapseOutput(text, 4, 500).output}
    </pre>
  );
}
  ...
  return (
    <div className="grid gap-2 px-3 py-2">
      {images.map((image, index) => {
        ...
        if (!source)
          return (
            <div key={image.id ?? index} className="text-xs text-muted-foreground">
              Tool returned an image without a displayable source.
            </div>
          );
        return (
          <a
      ...
      {structured !== undefined && (
        <DetailBlock label="Structured output">{JSON.stringify(structured, null, 2)}</DetailBlock>
      )}
      {unsupported.length > 0 && (
        <details className="text-xs text-muted-foreground">
          <summary>Unsupported result parts ({unsupported.length})</summary>
          <pre
            data-scrollable
            tabIndex={0}
            className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded-md bg-muted/45 p-2 font-mono text-xs"
          >
...
function ErrorText({ error }: { error: string }) {
  return (
    <pre className="ml-8 mt-1 whitespace-pre-wrap font-mono text-xs leading-5 text-destructive">
      {error}
    </pre>
  );
}
  ...
  return (
    <div>
      ...
      <pre
        data-scrollable
        tabIndex={0}
        className="max-h-96 overflow-auto whitespace-pre-wrap rounded-md bg-muted/45 p-2 font-mono text-xs leading-5"
      >
        {children}
      </pre>
    </div>
