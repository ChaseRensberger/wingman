...
function SessionDetailPage() {
  ...
  const transcriptScroll = useTranscriptScroll(
    `${session?.id}:${session?.history.length}:${visibleStreamingText.length}:${jsonMode}`,
    `${loading}:${session?.id}`,
  );
  const resetTranscriptScroll = useEffectEvent(() => transcriptScroll.reset());
      ...
      try {
        ...
        if (!cancelled) {
          ...
          if (agentsData.length > 0) {
            ...
            const variants =
              modelMap[modelRef.provider]?.find((model) => model.id === modelRef.model)?.variants ??
              [];
            const preferredVariant =
              modelRef.variant || storedModelVariant(modelRef.provider, modelRef.model);
            setSelectedVariant(
              preferredVariant && variants.includes(preferredVariant) ? preferredVariant : null,
            );
          }
        }
      } catch (err) {
    ...
    try {
      if (isDraft) {
        const created = (await client.sessions.create(
          draftWorkspaceId ? { workspace_id: draftWorkspaceId } : {},
        )) as SessionSummary;
  ...
  async function handleSaveSession(title: string, workDir: string) {
    ...
    try {
      ...
      if (title.trim() !== (updated.title ?? ""))
        updated = await renameSession(updated, title.trim());
  ...
  const selectedModelInfo = (models[selectedProvider] ?? []).find(
  ...
  const sessionTitle =
    ...
    (streamingTitle || isTitleStreaming ? "Generating title..." : session?.title);
  ...
  const modelCallsByMessageId = new Map(
    modelCalls
      .filter((call) => call.assistant_message_id)
      .map((call) => [call.assistant_message_id!, call]),
  );
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col bg-background">
      <SessionHeader
        ...
        onJsonModeChange={() => setJSONMode((value) => !value)}
        onCopy={(value, kind) => void copySessionValue(value, kind)}
        onEdit={openEditSession}
        onDelete={() => setDeleteSessionOpen(true)}
      />
      <SessionTranscript
        messages={transcriptHistory}
        rawMessages={session.history}
        jsonMode={jsonMode}
        greeting={greeting}
        ...
        toolActivitiesById={run.toolActivities}
        modelCallsByMessageId={modelCallsByMessageId}
        agentNames={agentNames}
        failedRun={run.failedRun}
        permissionRequests={run.permissionRequests}
        permissionRepliesInFlight={run.permissionRepliesInFlight}
        onReplyPermission={(requestID, response) => void handleReplyPermission(requestID, response)}
        copiedFailedRunError={copiedFailedRunError}
        onCopyFailedRunError={() => void copyFailedRunError()}
        onRetry={() => void handleSend(undefined, run.failedRun ?? undefined)}
      ...
      <SessionComposer
        ...
        onModelChange={(modelRef, selectedVariant) => {
          const ref = splitModelRef(modelRef);
          const variants =
            models[ref.provider]?.find((model) => model.id === ref.model)?.variants ?? [];
          const preferredVariant = storedModelVariant(ref.provider, ref.model);
          const variant =
            selectedVariant === undefined
              ? preferredVariant && variants.includes(preferredVariant)
                ? preferredVariant
                : null
              : selectedVariant;
          setSelectedProvider(ref.provider);
          setSelectedModel(ref.model);
          setSelectedVariant(variant);
          persistModelVariant(ref.provider, ref.model, variant);
          persistLastModelRef(buildModelRef(ref.provider, ref.model, variant));
        }}
        onSubmit={() => void handleSend()}
        onAbort={handleAbort}
        onJumpToBottom={transcriptScroll.jumpToBottom}
      />
      <SessionDialogs
        session={session}
        editing={editingSession}
        saving={savingSession}
        deleteOpen={deleteSessionOpen}
        deleting={deletingSession}
        onEditingChange={setEditingSession}
        onDeleteOpenChange={setDeleteSessionOpen}
        onSave={(title, workDir) => void handleSaveSession(title, workDir)}
