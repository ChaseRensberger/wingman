    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <Card size="sm" className="mb-6 px-3">
        ...
        {pluginErrors?.map((item) => (
          <div
            ...
            className="mb-3 rounded-[var(--radius)] border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive"
          >
            ...
            <TableBody>
              {plugins.map((plugin) => (
                <TableRow key={plugin.id}>
                  ...
                  <TableCell>
                    <Badge variant={pluginStatusVariant(plugin.status)}>{plugin.status}</Badge>
                  </TableCell>
                  <TableCell className="text-xs text-muted-foreground">
                    <div>{plugin.pid ? `PID ${plugin.pid}` : "No process"}</div>
                    <div>
                      {plugin.plugin_version ? `v${plugin.plugin_version}` : "Unversioned"},
                  ...
                  <TableCell className="font-mono text-xs text-muted-foreground">
                    {plugin.tools?.join(", ") || "None"}
                  </TableCell>
                  <TableCell className="max-w-sm text-xs text-muted-foreground">
                    {pluginDiagnostic(plugin)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      {loading ? (
        ...
        <Empty>
          ...
          <EmptyDescription>
            Native tools should appear here. Check server logs if the catalog is empty.
          </EmptyDescription>
        </Empty>
      ) : (
                ...
                <TableBody>
                  {items.map((tool) => (
                    <TableRow key={tool.name}>
                      ...
                      <TableCell className="font-mono text-xs text-muted-foreground">
                        {schemaSummary(tool.output_schema)}
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground">
                        {traitSummary(tool)}
                      </TableCell>
...
function schemaSummary(schema?: Record<string, unknown>) {
  ...
  if (!props || typeof props !== "object" || Array.isArray(props))
    return String(schema.type ?? "schema");
