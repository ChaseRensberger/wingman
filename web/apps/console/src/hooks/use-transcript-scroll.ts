...
export function useTranscriptScroll(content: unknown, resizeTarget: unknown) {
	...
	function updateScrollbar() {
		...
		setScrollbar((current) => {
			...
			const top = trackPadding + (maxScrollTop > 0 ? (element.scrollTop / maxScrollTop) * maxThumbTop : 0);
	...
	function handleScrollbarPointerDown(event: React.PointerEvent<HTMLDivElement>) {
		...
		scrollbarDragRef.current = { pointerId: event.pointerId, grabOffset: event.clientY - thumb.getBoundingClientRect().top };
	...
	function handleScrollbarPointerMove(event: React.PointerEvent<HTMLDivElement>) {
		...
		const thumbTop = Math.max(0, Math.min(event.clientY - element.getBoundingClientRect().top - trackPadding - drag.grabOffset, maxThumbTop));
