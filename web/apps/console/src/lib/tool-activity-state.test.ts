...
describe("tool activity state", () => {
  test("migrates pre-proposal input to its durable tool use", () => {
    let activities = reduceToolActivity(new Map(), {
      type: "input",
      run_id: "run-1",
      call_id: "call-1",
      tool: "read",
    ...
    expect(activities.get("tlu-1")).toMatchObject({
      tool_use_id: "tlu-1",
      run_id: "run-1",
      call_id: "call-1",
      input_text: '{"path":',
  ...
  test("keeps repeated provider call IDs distinct across durable tool uses", () => {
    ...
    expect(activities.get("tlu-2")).toMatchObject({
      run_id: "run-2",
      tool: "bash",
      status: "pending",
    });
    expect(
      lookupToolActivity(activities, { tool_use_id: "tlu-missing", call_id: "call-1" }),
    ).toBeUndefined();
  });
  ...
  test("normalizes every server lifecycle status", () => {
    ...
    for (const status of ["failed", "interrupted", "declined", "error"])
      expect(normalizeToolStatus(status)).toBe("error");
  });
