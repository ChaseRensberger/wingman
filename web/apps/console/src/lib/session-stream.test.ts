...
test("session titles use a short topic instead of a model's answer", () => {
  ...
  expect(
    generatedSessionTitle(
      "I don't have live internet access or real-time data feeds, so I can't help",
      message,
    ),
  ).toBe("Current weather in New York City");
  expect(generatedSessionTitle("", message)).toBe("Current weather in New York City");
  expect(
    generatedSessionTitle("A very long generated answer that goes far beyond seven words", message),
  ).toBe("Current weather in New York City");
});
