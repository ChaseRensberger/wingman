...
describe("unified diff display", () => {
	...
	test("reads file mutation metadata", () => {
		expect(patchFiles([{ relativePath: "a.go", movePath: "b.go", type: "move", patch: "@@ -1 +1 @@", additions: 1, deletions: 1 }])).toEqual([{ filePath: undefined, relativePath: "a.go", movePath: "b.go", type: "move", patch: "@@ -1 +1 @@", additions: 1, deletions: 1 }]);
	});

	test("does not mistake changed content for file headers", () => {
		const lines = parseUnifiedDiff("--- a/file\n+++ b/file\n@@ -1 +1 @@\n---old\n+++new\n");
		expect(lines.find((line) => line.kind === "deletion")).toEqual({ kind: "deletion", text: "--old", oldLine: 1 });
		expect(lines.find((line) => line.kind === "addition")).toEqual({ kind: "addition", text: "++new", newLine: 1 });
	});
});
