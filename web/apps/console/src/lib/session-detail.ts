...
export function reasoningSummary(reasoning: string) {
  ...
  if (boldHeading) {
    return {
      title: cleanReasoningHeading(boldHeading[1] ?? ""),
      body: markdown.slice(boldHeading[0].length).trim(),
    };
  }
  const heading = markdown.match(
    /^(?:<h[1-6][^>]*>([\s\S]*?)<\/h[1-6]>|\s{0,3}#{1,6}[ \t]+(.+?)(?:[ \t]+#+[ \t]*)?)\n?/,
  );
  if (heading) {
    return {
      title: cleanReasoningHeading(heading[1] ?? heading[2] ?? ""),
