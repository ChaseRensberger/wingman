...
export function toolActivityKey(identity: ToolIdentity): string | undefined {
	return stringField(identity.tool_use_id) ?? transientToolActivityKey(identity.run_id, identity.call_id);
}
...
export function reduceToolActivity(previous: Map<string, ToolActivity>, event: ToolActivityEvent): Map<string, ToolActivity> {
	const key = toolActivityKey(event);
	if (!key) return previous;
	const fallbackKey = stringField(event.tool_use_id) ? transientToolActivityKey(event.run_id, event.call_id) : undefined;
	...
	if (event.type === "input") {
		...
		next.set(key, { ...common, status: current?.status ?? "pending", input, input_text: inputText });
		return next;
	}

	if (event.type === "progress") {
		const eventMetadata = recordField(event.metadata);
		const metadata = eventMetadata ? { ...(current?.metadata ?? {}), ...eventMetadata } : current?.metadata;
		next.set(key, {
			...common,
...
export function lookupToolActivity(activities: Map<string, ToolActivity>, identity: ToolIdentity): ToolActivity | undefined {
	...
	return callID ? [...activities.values()].find((activity) => activity.call_id === callID) : undefined;
}
