...
export function parseUnifiedDiff(patch: string): DiffLine[] {
	...
	return patch.split("\n").flatMap((line, index, lines): DiffLine[] => {
		...
		if (!inHunk && (line.startsWith("---") || line.startsWith("+++"))) return [{ kind: "header", text: line }];
		...
		const result = { kind: "context" as const, text: line.startsWith(" ") ? line.slice(1) : line, oldLine, newLine };
		oldLine++;
		newLine++;
		return [result];
	});
