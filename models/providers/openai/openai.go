// Package openai implements models.Model for OpenAI's Responses API.
//
// Wire reference: https://platform.openai.com/docs/api-reference/responses
//
// Stream mapping (Responses SSE -> models.StreamPart):
//
...
func (c *Client) Info() models.ModelInfo {
	if info, ok := catalog.Get("openai", c.model); ok {
		info.API = models.APIOpenAIResponses
...
func (c *Client) buildRequest(req models.Request, caps models.ModelCapabilities) responsesRequest {
...
// toInputItems converts a models message slice to Responses API input items.
//
// Role mapping:
...
func (c *Client) toInputItems(req models.Request) []inputItem {
	var items []inputItem
...
func toolResultOutput(out []models.Part) any {
	if len(out) == 0 {
		return ""
	}
	// Fast path: single text → plain string.
	if len(out) == 1 {
		if t, ok := out[0].(models.TextPart); ok {
			return t.Text
		}
	}
...
type responseItem struct {
	kind     itemKind
	id       string // models stream-part id
...
func (p *streamParser) handle(eventType, data string) bool {
	...
	case "response.created":
		...
		p.responseMeta = models.ResponseMetadata{ID: ev.Response.ID}
	...
	case "response.reasoning_summary_text.delta":
		...
		p.out.Push(models.ReasoningDeltaPart{ID: item.id, Delta: ev.Delta})
	...
	case "response.output_text.delta":
		...
		item.textBuf.WriteString(ev.Delta)
		p.out.Push(models.TextDeltaPart{ID: item.id, Delta: ev.Delta})
	...
	case "response.function_call_arguments.delta":
		...
		p.out.Push(models.ToolInputDeltaPart{ID: item.callID, Delta: ev.Delta})
	...
	case "response.output_item.done":
		...
		case "reasoning":
			...
			p.out.Push(models.ReasoningEndPart{ID: item.id})

		case "message":
			p.content = append(p.content, models.TextPart{Text: item.textBuf.String()})
			p.out.Push(models.TextEndPart{ID: item.id})
		...
		case "function_call":
			...
			p.out.Push(models.ToolCallPart_{ID: callID, ToolName: item.toolName, Input: input})
		}
	...
	case "response.failed":
		...
		p.out.Push(models.ErrorPart{Message: msg, Code: ev.Response.Error.Code})
		p.terminate(models.FinishReasonError, fmt.Errorf("openai: %s", msg))
		return true
	...
	case "error":
		...
		_ = json.Unmarshal([]byte(data), &ev)
		p.out.Push(models.ErrorPart{Message: ev.Message, Code: ev.Code})
		p.terminate(models.FinishReasonError, fmt.Errorf("openai: %s: %s", ev.Code, ev.Message))
		return true
	}
	return false
}
...
func mapStopReason(status string) models.FinishReason {
	switch status {
	case "completed":
