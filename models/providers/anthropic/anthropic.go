// Package anthropic implements models.Model for Anthropic's Messages API.
//
// Wire reference: https://docs.anthropic.com/en/api/messages-streaming
//
// Stream mapping (Anthropic SSE -> models.StreamPart):
//
...
//	ping                   -> (skipped)
//
// Anthropic stop reasons map to models.FinishReason:
//
//	end_turn      -> stop
//	max_tokens    -> length
//	stop_sequence -> stop
...
//
// Factory returns a *Client typed as models.Model. Callers that need
...
func (c *Client) CountTokens(ctx context.Context, msgs []models.Message) (int, error) {
...
// models block id (we use a stable string derived from the index since
...
func (p *streamParser) handle(eventType, data string) bool {
	...
	case "message_start":
		...
		p.responseMeta = models.ResponseMetadata{ID: ev.Message.ID, ModelID: ev.Message.Model}
	...
	case "content_block_start":
		...
		case "tool_use":
			...
			p.out.Push(models.ToolInputStartPart{ID: id, ToolName: ev.ContentBlock.Name})
		default:
	...
	case "error":
		...
		_ = json.Unmarshal([]byte(data), &ev)
		p.out.Push(models.ErrorPart{Message: ev.Error.Message, Code: ev.Error.Type})
		p.terminate(models.FinishReasonError, fmt.Errorf("anthropic: %s: %s", ev.Error.Type, ev.Error.Message))
		return true
...
// blockID returns the models stream-part id for an Anthropic content
...
// They are NOT the models Message/Part shapes; they are the Anthropic
...
func (c *Client) buildRequest(req models.Request) builtRequest {
	...
	// ToolChoice
	// NOTE: disable_parallel_tool_use is not supported by the models
...
// toWireMessages converts models.Message to Anthropic's wire format.
//
// Role mapping:
//   - user, assistant -> same
...
func (c *Client) toWireMessages(msgs []models.Message) []anthropicMessage {
...
// for the input.
func toolResultContent(out []models.Part) any {
	if len(out) == 0 {
		return ""
	}
