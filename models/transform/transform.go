...
// pre-processing.
//
//	func (p *anthropicModel) Stream(ctx context.Context, req models.Request) (...) {
...
type Target struct {
	...
	// each assistant message's Origin (see models.MessageOrigin) to
...
func (t Target) origin() *models.MessageOrigin {
...
func Apply(msgs []models.Message, target Target) []models.Message {
	if len(msgs) == 0 {
	...
	rewritten := make([]models.Message, 0, len(pruned))
	...
	out := make([]models.Message, 0, len(withResults))
...
func buildToolCallIDRenames(msgs []models.Message, target Target, tgtOrigin *models.MessageOrigin) map[string]string {
...
func dropFailedAssistants(msgs []models.Message) []models.Message {
...
func rewriteMessage(
	msg models.Message,
	target Target,
	tgtOrigin *models.MessageOrigin,
	...
	case models.RoleAssistant:
		...
		newContent := make(models.Content, 0, len(msg.Content))
		for _, p := range msg.Content {
			switch part := p.(type) {
...
// present).
func downgradeImages(content []models.Part, placeholder string) ([]models.Part, bool) {
	hasImage := false
	...
	out := make([]models.Part, 0, len(content))
	prevWasPlaceholder := false
	for _, p := range content {
		if _, ok := p.(models.ImagePart); ok {
			if !prevWasPlaceholder {
				out = append(out, models.TextPart{Text: placeholder})
			}
		...
		if tp, ok := p.(models.TextPart); ok && tp.Text == placeholder {
			prevWasPlaceholder = true
		} else {
			prevWasPlaceholder = false
		}
...
func reconcileOrphanedToolCalls(msgs []models.Message) []models.Message {
	out := make([]models.Message, 0, len(msgs))
	...
	flushSynthetic := func() {
		for _, pc := range pendingCalls {
			...
			out = append(out, models.NewToolResult(
				pc.callID,
				[]models.Part{models.TextPart{Text: syntheticOrphanResult}},
				true,
			))
		}
		pendingCalls = nil
