// Package session is a thin stateful wrapper over wingharness/loop.
//
// A Session owns:
//   - an identifier (ULID)
...
//
// Plugins (wingharness/plugin) are opt-in: nothing is installed by
...
// transport. The caller (typically wingharness/server) wires those in by
