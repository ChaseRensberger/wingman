...
3. builds a `*session.Session` with the [storage plugin](./wingharness/storage#the-storage-plugin) installed via `session.WithPlugin(storage.NewPlugin(store, sess.ID))`
...
See [Storage](./wingharness/storage) for the schema and [Sessions](./wingharness/sessions) for what `Run` actually does.
...
The full envelope schema is in [Streaming](./wingharness/streaming).
