...
Also as an aside, I Wingman (as I've already mentioned) is written in golang and can thus has serveral features (like [WingModels](/concepts/wingmodels)) that can be imported and used as an SDK for your go application. This documentation is focused largely on the complete Wingman runtime but I suspect there will be people that want to use just the SDK without all the extra fluff. I will update the documentation in time to streamline that process.

## Next Steps

- **Want to start using Wingman?** Check out the [Quickstart](/start-here/quickstart)
