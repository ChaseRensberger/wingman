...
A `models.Message` is a role plus a list of `Part` values. `Part` is a discriminated union: each implementation reports a stable string discriminator via `Type()`, and the JSON serialization carries that discriminator as a `"type"` field.

## Built-in parts

The core types live in `models`:
...
The `Part` interface is sealed to the `models` package via an unexported marker, so no foreign Go type can implement it directly. The discriminator name `reasoning` matches Vercel AI SDK v3 and generalizes across Anthropic extended thinking, OpenAI o1/o3, and DeepSeek R1.
...
`OpaquePart` satisfies `Part` (its `isPart` marker lives in `models`) and round-trips its raw bytes verbatim. Plugins ship a typed accessor so consumers don't deal with raw JSON. The compaction plugin's `MarkerPart` is the canonical example:
...
`models.RegisterPart(typeName, decoder)` registers a discriminator with the global decoder registry. `UnmarshalPart` dispatches on `"type"`:
...
func (p *Plugin) Install(r *plugin.Registry) error {
    r.RegisterPart("my_marker", func(data []byte) (models.Part, error) {
        raw := make([]byte, len(data))
        copy(raw, data)
        return models.OpaquePart{TypeName: "my_marker", Raw: raw}, nil
    })
    // ... hooks, tools, sinks
    return nil
}
