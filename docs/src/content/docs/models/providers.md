...
A provider is any value that implements `models.Model`. The interface is small and provider-agnostic, so the rest of the runtime is independent of any specific backend.
...
`StreamText` returns a `Stream` whose `Next/Part/Err` surface emits `models.StreamPart` values matching Vercel AI SDK v3 exactly. See [Streaming](../models/streaming).
...
`models.Request` carries two first-class fields for caller-controlled behaviour:
...
result, err := loop.Run(ctx, loop.Config{
    Model:        model,
    Messages:     history,
    ToolChoice:   models.ToolChoice{Mode: models.ToolChoiceAuto},
    Capabilities: models.Capabilities{
        Thinking: &models.ThinkingConfig{BudgetTokens: 4096},
    },
})
```
...
|---|---|---|---|
| `anthropic` | `models/providers/anthropic` | Anthropic Messages API | Exact `CountTokens` via `/v1/messages/count_tokens`. |
| `ollama` | `models/providers/ollama` | Ollama Chat API | Local inference; `CountTokens` falls back to chars/4. |
| `openai` | `models/providers/openai` | OpenAI Responses API | `store: false`; full input each turn (stateless). `CountTokens` falls back to chars/4. |
| `opencodezen` | `models/providers/opencodezen` | OpenAI Chat Completions (proxy) | Multi-model proxy at `https://opencode.ai/zen/v1`. Auth: `OPENCODE_API_KEY`. Catalog key `opencode`. |
...
```go
import "github.com/chaserensberger/wingman/models/providers/ollama"
...
```go
import "github.com/chaserensberger/wingman/models/providers/openai"
...
```go
import "github.com/chaserensberger/wingman/models/providers/opencodezen"
