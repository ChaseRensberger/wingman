...
`*session.Session` is a thin wrapper over `wingharness/loop`. It holds state; the loop does the work.
...
`RunStream` runs the loop on a background goroutine and exposes a single-consumer iterator. See [Streaming](../wingharness/streaming).
