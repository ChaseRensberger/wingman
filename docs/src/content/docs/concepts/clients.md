...
Every persisted session and Base belongs to a client. If you omit `X-Wingman-Client`, Wingman uses the built-in default client named `Wingman` with ID `cli_wingman`, so manual `curl` calls and local scripts still work without setup.
...
Client identity also scopes Bases. `GET /bases` returns Bases for the active client and ensures that client's default `Wingman` Base exists.
