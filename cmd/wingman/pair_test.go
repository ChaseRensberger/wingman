...
func TestWritePairingInfo(t *testing.T) {
	info := pairingInfo{URLs: []string{"http://192.0.2.1:2424", "http://198.51.100.1:2424"}, Username: "wingman", Password: "secret"}
	payload, err := json.Marshal(info)
	if err != nil {
		t.Fatal(err)
	}
	if got, want := string(payload), `{"urls":["http://192.0.2.1:2424","http://198.51.100.1:2424"],"username":"wingman","password":"secret"}`; got != want {
	...
	for _, want := range []string{
		"URLs      http://192.0.2.1:2424",
		"            http://198.51.100.1:2424",
		"Username  wingman",
		"Password  secret",
		"Scan to pair",
		"█",
...
func TestWritePairingInfoIncludesRemoteHintForLoopbackURL(t *testing.T) {
	var output bytes.Buffer
	err := writePairingInfo(&output, pairingInfo{URLs: []string{"http://127.0.0.1:2424"}, Username: "wingman", Password: "secret"})
	if err != nil {
		t.Fatal(err)
	}
