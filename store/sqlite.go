...
// CreateAuthSession stores a hashed bearer token for a client.
func (s *SQLiteStore) CreateAuthSession(session *AuthSession) error {
	if session.ClientID == "" {
		return errors.New("auth session client ID is required")
	}
	if session.TokenHash == "" {
		return errors.New("auth session token hash is required")
	}
	...
	if session.ID == "" {
		session.ID = NewID(PrefixAuthSession)
	}
	if session.CreatedAt == "" {
		session.CreatedAt = Now()
	}
	if _, err := s.GetClient(session.ClientID); err != nil {
		return err
	}
	_, err := s.db.Exec(`
...
// AuthenticateAuthSession returns the active, unexpired session for tokenHash.
// It returns nil, nil when no such session can authenticate.
func (s *SQLiteStore) AuthenticateAuthSession(tokenHash string) (*AuthSession, error) {
	...
	if authSessionExpired(session.ExpiresAt) {
		return nil, nil
	}
	return &session, nil
}
...
// RevokeAuthSession prevents a session from authenticating. It is idempotent.
func (s *SQLiteStore) RevokeAuthSession(id string) error {
	_, err := s.db.Exec(`
